...
這套設定其實就是讓 Windows 監聽局域網 4200 埠，並把流量轉給 WSL 的 Next.js 服務。相比一般開發環境可以直接下 `--host` 就能讓手機連線，在 wsl 環境下又多了段轉發的設定，不過不需要額外搭建代理或改動其他埠號，對開發測試的影響非常小，只帶來一點點網路轉發延遲；而且規則會留存到系統重啟，若要還原只要刪除 portproxy 即可。
